...
function resolveSerializedUserDataField(
  value: SerializedUserDataField,
  context: Context
): string {
  return context.user.data?.[value.$userDataField] ?? ''
}
  ...
  switch (field.type) {
    ...
    case FieldType.FILE_WITH_OPTIONS:
      ...
      if (isSerializedUserDataField(defaultValue)) {
        return resolveSerializedUserDataField(defaultValue, context)
      }
